...
struct PlanarInertialEskfConfig {
...
namespace planar_inertial_eskf_detail {
...
inline void normalize(PlanarInertialEskfConfig& config) {
    ...
    config.gyro_noise_std = planar_inertial_eskf_detail::positiveOr(config.gyro_noise_std, 0.03);
    config.accel_noise_std = planar_inertial_eskf_detail::positiveOr(config.accel_noise_std, 0.35);
    config.gyro_bias_random_walk_std =
        planar_inertial_eskf_detail::nonNegativeOr(config.gyro_bias_random_walk_std, 1.0e-4);
    config.accel_bias_random_walk_std =
        planar_inertial_eskf_detail::nonNegativeOr(config.accel_bias_random_walk_std, 1.0e-3);
    config.extrinsic_position_random_walk_std =
        planar_inertial_eskf_detail::nonNegativeOr(config.extrinsic_position_random_walk_std, 1.0e-5);
    config.extrinsic_yaw_random_walk_std =
        planar_inertial_eskf_detail::nonNegativeOr(config.extrinsic_yaw_random_walk_std, 1.0e-5);
    config.vrpn_position_noise_std = planar_inertial_eskf_detail::positiveOr(config.vrpn_position_noise_std, 0.01);
    config.vrpn_yaw_noise_std = planar_inertial_eskf_detail::positiveOr(config.vrpn_yaw_noise_std, 0.01);
    config.innovation_position_gate_m = planar_inertial_eskf_detail::positiveOr(config.innovation_position_gate_m, 1.5);
    config.innovation_yaw_gate_rad = planar_inertial_eskf_detail::positiveOr(config.innovation_yaw_gate_rad, 0.8);
    config.covariance_high_threshold = planar_inertial_eskf_detail::positiveOr(config.covariance_high_threshold, 100.0);
    config.max_propagation_dt_s = planar_inertial_eskf_detail::positiveOr(config.max_propagation_dt_s, 0.05);
    config.initial_position_variance = planar_inertial_eskf_detail::positiveOr(config.initial_position_variance, 0.01);
    config.initial_velocity_variance = planar_inertial_eskf_detail::positiveOr(config.initial_velocity_variance, 0.1);
    config.initial_yaw_variance = planar_inertial_eskf_detail::positiveOr(config.initial_yaw_variance, 0.01);
    config.initial_gyro_bias_variance =
        planar_inertial_eskf_detail::positiveOr(config.initial_gyro_bias_variance, 0.01);
    config.initial_accel_bias_variance =
        planar_inertial_eskf_detail::positiveOr(config.initial_accel_bias_variance, 0.1);
    config.initial_extrinsic_position_variance =
        config.estimate_extrinsic
            ? planar_inertial_eskf_detail::positiveOr(config.initial_extrinsic_position_variance, 1.0e-4)
            : 1.0e-12;
    config.initial_extrinsic_yaw_variance =
        config.estimate_extrinsic
            ? planar_inertial_eskf_detail::positiveOr(config.initial_extrinsic_yaw_variance, 1.0e-4)
            : 1.0e-12;
}

inline PlanarInertialEskfConfig normalized(PlanarInertialEskfConfig config) {
    normalize(config);
    return config;
}

class PlanarInertialEskf {
  public:
    ...
    void setConfig(const PlanarInertialEskfConfig& config) {
    ...
    void initializeFromPose(const PlanarPoseMeasurement& pose, const PlanarInertialSample* inertial = nullptr) {
        if (!planar_inertial_eskf_detail::validPoseMeasurement(pose)) {
            return;
        }
        ...
        state_.last_inertial_stamp_sec =
            inertial != nullptr && planar_inertial_eskf_detail::validInertialSample(*inertial) ? inertial->stamp_sec
                                                                                               : pose.stamp_sec;
    ...
    PlanarInertialPropagationResult propagateInertial(const PlanarInertialSample& inertial) {
        ...
        if (!state_.initialized || !planar_inertial_eskf_detail::validInertialSample(inertial)) {
            return result;
        }
    ...
    PlanarPoseUpdateResult updatePose(const PlanarPoseMeasurement& pose) {
        ...
        const Eigen::Vector3d innovation =
            planar_inertial_eskf_detail::normalizedInnovation(se2Error(predicted_marker, marker_world));
  ...
  private:
    ...
    MeasurementCovariance measurementCovariance() const {
        ...
        R(0, 0) = config_.vrpn_position_noise_std * config_.vrpn_position_noise_std;
        R(1, 1) = config_.vrpn_position_noise_std * config_.vrpn_position_noise_std;
        R(2, 2) = config_.vrpn_yaw_noise_std * config_.vrpn_yaw_noise_std;
        return R;
    }
    ...
    MeasurementMatrix numericalMeasurementJacobian(const Pose2& nominal_prediction,
        ...
        const Eigen::Vector3d nominal_residual =
            planar_inertial_eskf_detail::normalizedInnovation(se2Error(nominal_prediction, measured_marker));
        for (int i = 0; i < kErrorStateDim; ++i) {
            ...
            const Eigen::Vector3d perturbed_residual = planar_inertial_eskf_detail::normalizedInnovation(
    ...
    void propagateCovariance(const Eigen::Vector2d& accel_body, double dt) {
        ...
        const Eigen::Vector2d accel_yaw_derivative =
            rotation * planar_inertial_eskf_detail::yawDerivativeMatrix() * accel_body;
    ...
    PlanarInertialEskfConfig config_{};
