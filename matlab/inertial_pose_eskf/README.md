...
`run_pose3_inertial_eskf_simulation.m` mirrors the C++ observer tests for the
ROS-independent `xgc2_math::Pose3InertialEskf` algorithm. It uses the same
...
```matlab
results = run_pose3_inertial_eskf_simulation();
disp(results)
```
